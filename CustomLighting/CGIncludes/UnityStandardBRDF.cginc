    ...
    UnityLight light, UnityIndirect gi)
{
    ...
    half3 specularColor = CookTorranceBRDF(nh, nl, nv, lh, roughness, specColor);   
    #if UNITY_PASS_FORWARDADD
     half3 color =   (diffColor * diffuseTerm + specularColor) * light.color;
    #else
    half3 PreintegratedGF = PreintegratedGF_LUT(_PreintegratedLUT, specColor, perceptualRoughness, nv);
    half grazingTerm = saturate(smoothness + (1-oneMinusReflectivity));
    half3 color =   diffColor * (gi.diffuse + light.color * diffuseTerm)
                    + specularColor * light.color
                    + PreintegratedGF * gi.specular;
    #endif
